...
function Card({ title, subtitle, right, children }: any) {
  return (
    <div style={{ border: "1px solid rgba(0,0,0,0.08)", borderRadius: 16, padding: 14, marginTop: 14, background: "#fff" }}>
...
function Pill({ tone = "neutral", text }: { tone?: "high" | "med" | "low" | "info" | "neutral" | "chip"; text: string }) {
  const c =
    tone === "high"
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "4px 10px", borderRadius: 999, fontSize: 12, fontWeight: 900, background: c.bg, border: `1px solid ${c.border}`, whiteSpace: "nowrap" }}>
...
function sortSignals(sig: Signal[]) {
  ...
  return [...sig].sort((a, b) => {
    const ac = typeof a.confidence_0_100 === "number" ? a.confidence_0_100 : 0;
    const bc = typeof b.confidence_0_100 === "number" ? b.confidence_0_100 : 0;
    return (bc - ac) || ((rank[severityTone(b.severity)] ?? 0) - (rank[severityTone(a.severity)] ?? 0));
  });
}
...
function SignalRow({ s, showMeta }: { s: Signal; showMeta: boolean }) {
  ...
  return (
    <div style={{ border: "1px solid rgba(0,0,0,0.08)", borderRadius: 14, padding: 12, background: bg, display: "grid", gap: 6 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
          ...
          {typeof s.confidence_0_100 === "number" ? <Pill tone="chip" text={`CONF ${s.confidence_0_100}`} /> : null}
          <Pill tone={tone} text={tone.toUpperCase()} />
        </div>
      </div>

      {s.trigger ? <div style={{ fontSize: 13, color: "rgba(0,0,0,0.75)" }}><b>Trigger:</b> {s.trigger}</div> : null}
      {s.action ? <div style={{ fontSize: 13 }}><b>Action:</b> {s.action}</div> : null}
      {s.invalidation ? <div style={{ fontSize: 13, color: "rgba(0,0,0,0.75)" }}><b>Invalidation:</b> {s.invalidation}</div> : null}

      {Array.isArray(s.levels) && s.levels.length ? (
        <div style={{ fontSize: 13, color: "rgba(0,0,0,0.75)" }}><b>Levels:</b> {s.levels.slice(0, 6).map((x) => fmt(x)).join(", ")}</div>
      ) : null}

      {Array.isArray(s.confidence_notes) && s.confidence_notes.length ? (
        <div style={{ fontSize: 12, color: "rgba(0,0,0,0.6)" }}>
          <b>Confidence notes:</b> {s.confidence_notes.join(" • ")}
        </div>
      ) : null}
    ...
    <div>
      <Card title="Ask the engine" subtitle="Run a daily report for a symbol and get decision-ready signals">
        <div style={{ display: "flex", gap: 8 }}>
          <input value={symbol} onChange={(e) => setSymbol(e.target.value)} placeholder="TSLA" style={{ width: 90, padding: 10, borderRadius: 12, border: "1px solid rgba(0,0,0,0.14)" }} />
          <button
            onClick={run}
            disabled={loading}
            style={{
        ...
        <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={3} style={{ width: "100%", marginTop: 8, padding: 10, borderRadius: 12, border: "1px solid rgba(0,0,0,0.14)" }} />

        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10, alignItems: "center" }}>
          <span style={{ fontSize: 12, color: "rgba(0,0,0,0.55)", marginRight: 6, fontWeight: 900 }}>Signals view:</span>
          <ButtonChip active={severityFilter === "high_med"} onClick={() => setSeverityFilter("high_med")}>High + Med</ButtonChip>
          <ButtonChip active={severityFilter === "high"} onClick={() => setSeverityFilter("high")}>High only</ButtonChip>
          <ButtonChip active={severityFilter === "all"} onClick={() => setSeverityFilter("all")}>All</ButtonChip>

          <span style={{ width: 1, height: 18, background: "rgba(0,0,0,0.10)", margin: "0 6px" }} />

          <ButtonChip active={showMeta} onClick={() => setShowMeta((v: boolean) => !v)}>{showMeta ? "Hide debug" : "Show debug"}</ButtonChip>
        </div>
      ...
      {out && (
        <>
          ...
          <Card
            title="Signals"
            subtitle={rawSignals.length ? `Showing ${filteredSignals.length} of ${rawSignals.length} signal(s) — ranked by confidence` : "No signals in payload yet. Deploy backend changes and rerun."}
            right={
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
                <ButtonChip active={catFilter === "all"} onClick={() => setCatFilter("all")}>All ({rawSignals.length || 0})</ButtonChip>
                {categories.map((k) => (
