      ...
      {out && (
        <>
          ...
          <Card
            ...
            <div style={{ marginBottom: 10, padding: 10, borderRadius: 12, border: "1px dashed rgba(0,0,0,0.18)", background: "rgba(0,0,0,0.02)" }}>
              <div style={{ fontSize: 12, fontWeight: 900, marginBottom: 6 }}>Backend debug</div>
              <div style={{ fontSize: 12, color: "rgba(0,0,0,0.70)" }}>
                <div><b>payload keys:</b> {out ? Object.keys(out).join(", ") : "—"}</div>
                <div><b>signals present:</b> {Array.isArray(out?.signals) ? "yes" : "no"}</div>
                <div><b>_signals_debug:</b> {out?._signals_debug ? "yes" : "no"}</div>
                {out?._signals_debug ? (
                  <div style={{ marginTop: 6 }}>
                    <div><b>bars_len:</b> {out._signals_debug.bars_len}</div>
                    <div><b>signals_len:</b> {out._signals_debug.signals_len}</div>
                    {out._signals_debug.signals_error ? <div style={{ color: "crimson" }}><b>signals_error:</b> {String(out._signals_debug.signals_error)}</div> : null}
                  </div>
                ) : null}
              </div>
            </div>
